...
az container create \
    ...
    --environment-variables 'STORAGE_ACCOUNT_URL=https://devtimstoregrp1.blob.core.windows.net' \
                            'STORAGE_CONTAINER_NAME=testcontainer' \
                            'DOWNLOAD_TEMP_DIR=/tmp/http_rtrvr_temp' \
                            'TARGET_RTRVL_URL=https://example.com/index.html' \
                            'TARGET_RTRVL_SAVE_TO=env_tst_script' \
                            'TARGET_RTRVL_FILE_TYPE=simple' \
                            'TARGET_RTRVL_TIMEOUT_SECS=20' \
